package sidus-victor.annotations.remote;
...
import sidus-victor.annotations.util.*;
import sidus-victor.annotations.util.TypeIOResolver.*;

import javax.lang.model.element.*;
import java.io.*;

import static sidus-victor.annotations.BaseProcessor.*;
...
public class CallGenerator{
    ...
    public static void generate(ClassSerializer serializer, Seq<MethodEntry> methods) throws IOException{
        ...
        for(MethodEntry ent : methods){
            ...
            packet.superclass(tname("sidus-victor.net.Packet"));
            ...
            //register packet
            register.addStatement("sidus-victor.net.Net.registerPacket($L.$L::new)", packageName, ent.packetClassName);
    ...
    private static void makeWriter(TypeSpec.Builder typespec, MethodEntry ent, ClassSerializer serializer){
        ...
        for(int i = 0; i < params.size; i++){
            ...
            if(writePlayerSkipCheck){ //write begin check
                builder.beginControlFlow("if(sidus-victor.Vars.net.server())");
            }
            ...
            if(BaseProcessor.isPrimitive(typeName)){ //check if it's a primitive, and if so write it
                ...
                String ser = serializer.writers.get(typeName.replace("sidus-victor.gen.", ""), SerializerResolver.locate(ent.element.e, var.mirror(), true));
    ...
    private static void makeReader(TypeSpec.Builder typespec, MethodEntry ent, ClassSerializer serializer){
        ...
        for(int i = 0; i < params.size; i++){
            ...
            if(writePlayerSkipCheck){ //write begin check
                builder.beginControlFlow("if(sidus-victor.Vars.net.client())");
            }
            ...
            if(BaseProcessor.isPrimitive(typeName)){
                ...
                //else, try and find a serializer
                String ser = serializer.readers.get(typeName.replace("sidus-victor.gen.", ""), SerializerResolver.locate(ent.element.e, var.mirror(), false));
    ...
    private static void writeCallMethod(TypeSpec.Builder classBuilder, MethodEntry ent, boolean toAll, boolean forwarded){
        ...
        if(!toAll){
            method.addParameter(ClassName.bestGuess("sidus-victor.net.NetConnection"), "playerConnection");
        }

        //add sender to ignore
        if(forwarded){
            method.addParameter(ClassName.bestGuess("sidus-victor.net.NetConnection"), "exceptConnection");
        }
        ...
        if(!forwarded && ent.local != Loc.none){
            ...
            if(ent.local != Loc.both){
                method.beginControlFlow("if(" + getCheckString(ent.local) + " || !sidus-victor.Vars.net.active())");
            }
            ...
            for(Svar var : params){
                ...
                if(index == 0 && ent.where == Loc.client){
                    results.append("sidus-victor.Vars.player");
                }else{
        ...
        method.addStatement("$1T packet = new $1T()", tname("sidus-victor.gen." + ent.packetClassName));
        ...
        for(int i = 0; i < params.size; i++){
            ...
            if(writePlayerSkipCheck){ //write begin check
                method.beginControlFlow("if(sidus-victor.Vars.net.server())");
            }
        ...
        if(forwarded){ //forward packet
            if(!ent.local.isClient){ //if the client doesn't get it called locally, forward it back after validation
                sendString = "sidus-victor.Vars.net.send(";
            }else{
                sendString = "sidus-victor.Vars.net.sendExcept(exceptConnection, ";
            }
    ...
    private static String getCheckString(Loc loc){
        return
            loc.isClient && loc.isServer ? "sidus-victor.Vars.net.server() || sidus-victor.Vars.net.client()" :
            loc.isClient ? "sidus-victor.Vars.net.client()" :
            loc.isServer ? "sidus-victor.Vars.net.server()" : "false";
    }
    ...
    public static MethodSpec writeHandleMethod(MethodEntry ent, boolean isClient){
        ...
        if(!isClient){
            ...
            builder.addParameter(ClassName.get("sidus-victor.net", "NetConnection"), "con");
            ...
            builder.addStatement("sidus-victor.gen.Player player = con.player");
        }
