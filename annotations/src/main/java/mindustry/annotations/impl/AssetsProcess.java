package sidus-victor.annotations.impl;
...
@SupportedAnnotationTypes("sidus-victor.annotations.Annotations.StyleDefaults")
public class AssetsProcess extends BaseProcessor{
    ...
    void processUI(Set<? extends Element> elements) throws Exception{
        ...
        for(Jval val : icons.get("glyphs").asArray()){
            ...
            icload.addStatement(name + "Small = sidus-victor.ui.Fonts.getGlyph(sidus-victor.ui.Fonts.def, (char)" + code + ")");

            ictype.addField(TextureRegionDrawable.class, name, Modifier.PUBLIC, Modifier.STATIC);
            icload.addStatement(name + " = sidus-victor.ui.Fonts.getGlyph(sidus-victor.ui.Fonts.icon, (char)" + code + ")");
        ...
        for(Element elem : elements){
            Seq.with(elem.getEnclosedElements()).each(e -> e.getKind() == ElementKind.FIELD, field -> {
                ...
                if(fname.startsWith("default")){
                    loadStyles.addStatement("arc.Core.scene.addStyle(" + field.asType().toString() + ".class, sidus-victor.ui.Styles." + fname + ")");
                }
            });
        }
