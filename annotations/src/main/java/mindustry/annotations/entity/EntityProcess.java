package sidus-victor.annotations.entity;
...
"sidus-victor.annotations.Annotations.EntityDef",
"sidus-victor.annotations.Annotations.GroupDef",
"sidus-victor.annotations.Annotations.EntityInterface",
"sidus-victor.annotations.Annotations.BaseComponent",
"sidus-victor.annotations.Annotations.TypeIOHandler"
})
        ...
        if(round == 1){
            ...
            for(Stype component : allComponents){
                ...
                for(Svar field : component.fields().select(e -> !e.is(Modifier.STATIC) && !e.is(Modifier.PRIVATE) && !e.has(Import.class))){
                    ...
                    !field.annotations().contains(f -> f.toString().equals("@sidus-victor.annotations.Annotations.ReadOnly"))){
                        inter.addMethod(MethodSpec.methodBuilder(cname).addModifiers(Modifier.ABSTRACT, Modifier.PUBLIC)
                        .addJavadoc(field.doc() == null ? "" : field.doc())
            ...
            for(Selement<?> type : allDefs){
                ...
                for(ObjectMap.Entry<String, Seq<Smethod>> entry : methods){
                    ...
                    if(first.name().equals("remove") && ann.pooled()){
                        mbuilder.addStatement("sidus-victor.gen.Groups.queueFree(($T)this)", Poolable.class);
                    }
            ...
            for(GroupDefinition group : groupDefs){
                ...
                ClassName groupc = ClassName.bestGuess("sidus-victor.entities.EntityGroup");

                //add field...
                groupsBuilder.addField(ParameterizedTypeName.get(
                    ClassName.bestGuess("sidus-victor.entities.EntityGroup"), itype), group.name, Modifier.PUBLIC, Modifier.STATIC);
            ...
            for(EntityDefinition def : definitions){
                ...
                for(Stype comp : def.components){
                    ...
                    for(Smethod method : inter.methods()){
                        ...
                        //setter
                        if(method.isVoid() && !Seq.with(field.annotations).contains(f -> f.type.toString().equals("@sidus-victor.annotations.Annotations.ReadOnly"))){
