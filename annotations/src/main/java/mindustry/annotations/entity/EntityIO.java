...
import static sidus-victor.annotations.BaseProcessor.instanceOf;
...
public class EntityIO{
    ...
    final static StringMap replacements = StringMap.of("sidus-victor.entities.units.BuildRequest", "sidus-victor.entities.units.BuildPlan");
    ...
    private void io(String type, String field) throws Exception{
        type = type.replace("sidus-victor.gen.", "");
        type = replacements.get(type, type);

        if(BaseProcessor.isPrimitive(type)){
            s(type.equals("boolean") ? "bool" : type.charAt(0) + "", field);
        }else if(instanceOf(type, "sidus-victor.ctype.Content")){
            if(write){
                s("s", field + ".id");
            }else{
                st(field + "sidus-victor.Vars.content.getByID(sidus-victor.ctype.ContentType.$L, read.s())", BaseProcessor.simpleName(type).toLowerCase().replace("type", ""));
            }
    ...
    public static class Revision{
        ...
        boolean equal(Seq<FieldSpec> specs){
            ...
            for(int i = 0; i < fields.size; i++){
                ...
                if(!field.type.replace("sidus-victor.gen.", "").equals(spec.type.toString().replace("sidus-victor.gen.", ""))){
                    return false;
                }
            }
            return true;
