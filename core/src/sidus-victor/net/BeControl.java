...
import sidus-victor.net.Administration.*;
import sidus-victor.net.Packets.*;
...
public class BeControl{
    ...
    public void checkUpdate(Boolc done){
        Http.get("https://api.github.com/repos/Anuken/sidus-victorBuilds/releases/latest")
        .error(e -> {}) //ignore errors
        .submit(res -> {
            Jval val = Jval.read(res.getResultAsString());
            int newBuild = Strings.parseInt(val.getString("tag_name", "0"));
            if(newBuild > Version.build){
                Jval asset = val.get("assets").asArray().find(v -> v.getString("name", "").startsWith(headless ? "sidus-victor-BE-Server" : "sidus-victor-BE-Desktop"));
