...
import sidus-victor.graphics.MultiPacker.*;
import sidus-victor.mod.ContentParser.*;
    ...
    public void load(){
        ...
        for(Fi file : files){
            ...
            try{
                ...
                if(e instanceof ClassNotFoundException && e.getMessage().contains("sidus-victor.plugin.Plugin")){
                    Log.info("Plugin @ is outdated and needs to be ported to 6.0! Update its main class to inherit from 'sidus-victor.mod.Plugin'. See https://sidus-victorgame.github.io/wiki/modding/6-migrationv6/");
                }else{
    ...
    private LoadedMod loadMod(Fi sourceFile, boolean overwrite) throws Exception{
        ...
        try{
            ...
            if(
                ...
                //detect mods that incorrectly package sidus-victor in the jar
                if((main.getSuperclass().getName().equals("sidus-victor.mod.Plugin") || main.getSuperclass().getName().equals("sidus-victor.mod.Mod")) &&
                    main.getSuperclass().getClassLoader() != Mod.class.getClassLoader()){
                    throw new ModLoadException(
                        "This mod/plugin has loaded sidus-victor dependencies from its own class loader. " +
                        "You are incorrectly including sidus-victor dependencies in the mod JAR - " +
                        "make sure sidus-victor is declared as `compileOnly` in Gradle, and that the JAR is created with `runtimeClasspath`!"
                    );
                }
